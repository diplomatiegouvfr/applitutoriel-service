...
- Dézipper les fichiers de configuration présents dans l'archive `applitutoriel-service-5.4.X-config.zip`.
- Déployer l'application dans un conteneur Tomcat 9, en prenant garde que la variable `conf/applitutorielprop` du fichier `context.xml` de l'application, référence bien le chemin des fichiers de configuration dézippés. 
...
<configuration scan="true">
	<property name="repertoire" value="/var/log/tomcat9/applitutoriel-service" />
